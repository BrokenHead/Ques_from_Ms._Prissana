...
function getClockAngle(hh_mm) {
    ...
    let short_a = ((Number(hh_mm.split(":")[0]) % 12) + (Number(hh_mm.split(":")[1]) / 60)) * 30; //get angle for long hands 12 * 30 = 360 deg
    let long_a = Number(hh_mm.split(":")[1]) * 6; //get angle for short hands 60 * 6 = 360 deg
    let ans = Math.abs(short_a - long_a); //more - less
