...
function getQuestionPart(phrases) {
    ...
    let shortest = phrases[0].length;
    for (let i = 1; i < 3; i++) { //find length of shortest string        checked
        if (phrases[i].length < shortest)
            shortest = phrases[i].length;
    }
    ...
    return [];
    function substring(gotword, cut_length) {
        let array_string = [];
    ...
    function get_share(s_array) {
        let test_set = s_array[0];
        let share_times = 0;
